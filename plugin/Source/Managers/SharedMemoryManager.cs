...
// Field order must match scripts/read_trace.py HERO_PRIVATE_DTYPE.
[StructLayout(LayoutKind.Sequential, Pack = 4)]
public unsafe struct HeroPrivate
{
    public uint  cStateBits;
    public int   heroState;
    public int   transitionState;
    public int   jumpSteps;
    public int   jumpedSteps;
    public int   doubleJumpSteps;
    public float dashTimer;
    public float dashTime;
    public float dashCooldownTimer;
    public byte  airDashed;
    public byte  doubleJumped;
    public byte  didAirHang;
    public byte  controlReqlinquished;
    public int   ledgeBufferSteps;
    public int   sprintBufferSteps;
    ...
    public int   wallJumpChainStepsLeft;
    ...
    public float shuttlecockSpeed;
    public int   shuttleCockJumpSteps;
    public int   recoilStepsLeft;
    public float recoilVelocity;
    public float recoilTimer;
    public uint  extraBits;
    public uint  sprintFsmStateHash;
    public float sprintAddSpeed;
    public int   extraAirMoveCount;
    public float ext0Vx; public float ext0Vy; public float ext0Decay; public uint ext0Flags;
    public float ext1Vx; public float ext1Vy; public float ext1Decay; public uint ext1Flags;
    public float ext2Vx; public float ext2Vy; public float ext2Decay; public uint ext2Flags;
    public float ext3Vx; public float ext3Vy; public float ext3Decay; public uint ext3Flags;
}

public enum CStateBit : int
{
    FacingRight = 0, OnGround, Jumping, Falling, Dashing,
...
public enum ExtraBit : int
{
    WallJumpedL = 0, WallJumpedR, WallSlidingL, WallSlidingR,
...
public class SharedMemoryManager : MonoBehaviour
{
    ...
    public void WriteGameState()
    {
        try
        {
            ...
            TraceRecorder.Instance?.RecordStep(gameState);
        }
        catch (Exception e)
        {
    ...
    private void Update()
    {
        if (CommandLineArgs.Manual || CommandLineArgs.ReplayActive)
            return;
    ...
    private void FixedUpdate()
    {
        ...
        if (!CommandLineArgs.Manual) return;
        if (TraceRecorder.Instance == null || !TraceRecorder.Instance.IsRecording) return;

        ActionManager.IsLeftPressed     = Input.GetKey(KeyCode.LeftArrow);
        ActionManager.IsRightPressed    = Input.GetKey(KeyCode.RightArrow);
        ActionManager.IsUpPressed       = Input.GetKey(KeyCode.UpArrow);
        ActionManager.IsDownPressed     = Input.GetKey(KeyCode.DownArrow);
        ActionManager.IsJumpPressed     = Input.GetKey(KeyCode.Z);
        ActionManager.IsAttackPressed   = Input.GetKey(KeyCode.X);
        ActionManager.IsDashPressed     = Input.GetKey(KeyCode.C);
        ActionManager.IsClawlinePressed = Input.GetKey(KeyCode.S);
        ActionManager.IsSkillPressed    = Input.GetKey(KeyCode.F);
        ActionManager.IsHealPressed     = Input.GetKey(KeyCode.A);
